...
export function Sidebar({ currentView, onSelect, hiddenViews }: SidebarProps) {
  const items = hiddenViews
    ...
    : NAV_ITEMS;
  return (
    <aside className="hidden lg:flex w-56 shrink-0 border-r border-[var(--color-border)] bg-[var(--color-surface-muted)] flex-col">
      ...
      <div className="p-3 border-t border-[var(--color-border)] text-[10px] text-[var(--color-muted-foreground)]">
...
/**
 * Mobile bottom navigation bar (visible below `lg` breakpoint).
 * Horizontally scrollable to accommodate many nav items.
 */
