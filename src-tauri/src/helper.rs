...
/// Load TTS components
pub fn load_text_to_speech<P: AsRef<Path>>(onnx_dir: P, use_gpu: bool) -> Result<TextToSpeech> {
    if use_gpu {
...
mod tests {
    ...
    /// Returns the model directory used for tests.
    /// Uses `<project_root>/test-model` so it persists between runs.
    ...
    /// Ensure the model files are downloaded before the load test runs.
    fn ensure_model_downloaded(model_dir: &Path) {
        if !model_downloader::is_model_downloaded(model_dir) {
            println!("Model not found at {:?}, downloading...", model_dir);
            model_downloader::download_model(model_dir, |current, total, file| {
                println!("[{}/{}] Downloading {}", current + 1, total, file);
            })
            .expect("Failed to download model");
            println!("Model downloaded successfully.");
        } else {
            println!("Model already present at {:?}.", model_dir);
        }
    }

    #[test]
    fn test_load_text_to_speech_within_one_minute() {
        ...
        assert!(
            onnx_dir.exists(),
            "ONNX directory should exist at {:?}",
            onnx_dir
        );

        let start = std::time::Instant::now();
        let result = load_text_to_speech(&onnx_dir, false);
        let elapsed = start.elapsed();

        assert!(
            result.is_ok(),
            "load_text_to_speech failed: {:?}",
            result.err()
        );

        let tts = result.unwrap();
        assert!(tts.sample_rate > 0, "sample_rate should be positive");

        assert!(
            elapsed < Duration::from_secs(60),
            "load_text_to_speech took {:?}, which exceeds the 60-second limit",
            elapsed
        );

        println!(
            "load_text_to_speech completed in {:.2}s",
            elapsed.as_secs_f64()
        );
    }
}
